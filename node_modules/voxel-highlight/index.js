...
function Highlighter(game, opts) {
  ...
  this.game = game
  opts = opts || {}
  this.enabled = opts.enabled || function () {
    return true
  }
  ...
  var material = new game.THREE.LineBasicMaterial({
    color: 0xff0000
  });
  var geometry = new game.THREE.Geometry();
  geometry.vertices.push(
    new game.THREE.Vector3(0, 0.5, 0),
    new game.THREE.Vector3(0.8, 0.5, 0)
  );
  var x = new game.THREE.Line(geometry, material);
  this.mesh.add(x);
  ...
  geometry.vertices.push(
    new game.THREE.Vector3(0, 0.5, 0),
    new game.THREE.Vector3(0, 1.3, 0)
  );
  var y = new game.THREE.Line(geometry, material);
  this.mesh.add(y);
  ...
  geometry.vertices.push(
    new game.THREE.Vector3(0, 0.5, 0),
    new game.THREE.Vector3(0, 0.5, 0.8)
  );
  var z = new game.THREE.Line(geometry, material);
  this.mesh.add(z);
  ...
  this.animateFunction = opts.animateFunction || function (position, targetPosition, deltaTime) {
    ...
    var rate = 10
    if (Math.abs(targetPosition[0] - position[0]) < 0.05 && Math.abs(targetPosition[1] - position[1]) < 0.05 && Math.abs(targetPosition[2] - position[2]) < 0.05) {
...
Highlighter.prototype.highlight = function () {
  ...
  if (!this.currVoxelPos || newVoxelPos[0] !== this.currVoxelPos[0] || newVoxelPos[1] !== this.currVoxelPos[1] || newVoxelPos[2] !== this.currVoxelPos[2]) { // no current highlight or it moved
  ...
  if (this.adjacentActive()) {
    ...
    if (!this.currVoxelAdj || newVoxelAdj[0] !== this.currVoxelAdj[0] || newVoxelAdj[1] !== this.currVoxelAdj[1] || newVoxelAdj[2] !== this.currVoxelAdj[2]) { // no current adj highlight or it has moved
  ...
  if (this.selectActive()) {
    if (!this.selectStart) { // start a new selection
      ...
      if (endCandidate[0] !== this.selectEnd[0] || endCandidate[1] !== this.selectEnd[1] || endCandidate[2] !== this.selectEnd[2]) {
